...
<body class="bg-slate-50 text-slate-900">
    <header class="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-teal-600 to-slate-900 text-white">
        <div class="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(255,255,255,0.15),transparent_35%),radial-gradient(circle_at_80%_0%,rgba(255,255,255,0.08),transparent_25%)]"></div>
        <div class="relative max-w-6xl mx-auto px-6 py-16 lg:py-24">
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                <div>
                    @php $logoUrl = $identity->getFirstMediaUrl('logo', 'thumb'); @endphp
                    @if($logoUrl)
                    <img src="{{ $logoUrl }}" alt="Logo {{ $identity->nama_desa ?? 'Desa' }}" class="h-14 w-auto mb-4 drop-shadow-lg">
                    @endif
                    <p class="uppercase tracking-wide text-sm font-semibold text-emerald-100">Identitas {{ $identity->nama_desa ?? 'Desa' }}</p>
                    <h1 class="mt-3 text-4xl lg:text-5xl font-bold leading-tight">Profil {{ $identity->nama_desa ?? 'Desa' }} yang transparan, modern, dan dekat dengan warga.</h1>
                    <p class="mt-5 text-lg text-emerald-50">Kenali potensi, program, dan layanan publik {{ $identity->nama_desa ?? 'desa' }} dalam satu halaman landing yang ringkas namun informatif.</p>
                    <div class="mt-8 flex flex-wrap gap-3">
                        <a href="#layanan" class="px-5 py-3 rounded-lg bg-white text-emerald-700 font-semibold shadow-lg shadow-emerald-900/20 hover:-translate-y-0.5 transition">Lihat Layanan</a>
                        <a href="#program" class="px-5 py-3 rounded-lg border border-emerald-200 text-white hover:bg-white/10 transition">Program Unggulan</a>
                    </div>
                </div>
                <div class="bg-white/10 border border-white/20 rounded-2xl backdrop-blur p-6 shadow-2xl">
                    ...
                    <div class="mt-6 rounded-xl bg-white/10 p-4">
                        ...
                        <p class="text-lg font-semibold">Dipimpin oleh {{ $identity->kepala_desa ?? 'Kepala Desa' }} dengan komitmen pelayanan publik.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <main class="max-w-6xl mx-auto px-6 py-16 space-y-20">
        <section id="profil" class="grid md:grid-cols-2 gap-10 items-center">
            <div class="space-y-4">
                ...
                <p class="text-slate-600 leading-relaxed">Kantor desa beralamat di {{ $identity->alamat_kantor ?? 'alamat belum diisi' }}. Informasi kontak: {{ $identity->email_desa ?? 'email belum diisi' }} / {{ $identity->telepon_desa ?? 'telepon belum diisi' }}.</p>
                <ul class="space-y-3 text-slate-700">
                    <li class="flex gap-3"><span class="text-emerald-600">•</span> Layanan publik digital dan terintegrasi.</li>
                    <li class="flex gap-3"><span class="text-emerald-600">•</span> Program ekonomi kerakyatan untuk UMKM dan petani.</li>
                    <li class="flex gap-3"><span class="text-emerald-600">•</span> Pelestarian budaya dan ruang terbuka hijau.</li>
                </ul>
            </div>
        ...
        <section id="layanan" class="space-y-6">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div class="p-6 rounded-2xl bg-white shadow-lg border border-slate-100">
                    ...
                    <p class="mt-2 text-slate-600">Surat pengantar, pindah datang, KK, KTP, akta, dengan jadwal layanan daring.</p>
                    <span class="inline-block mt-4 px-3 py-1 bg-emerald-50 text-emerald-700 text-sm rounded-full">Layanan Online</span>
                </div>
                <div class="p-6 rounded-2xl bg-white shadow-lg border border-slate-100">
                    <h3 class="text-xl font-semibold text-slate-900">Perizinan Usaha & UMKM</h3>
                    <p class="mt-2 text-slate-600">Pendampingan perizinan, sertifikasi halal, dan promosi produk unggulan desa.</p>
                    <span class="inline-block mt-4 px-3 py-1 bg-amber-50 text-amber-700 text-sm rounded-full">UMKM</span>
                </div>
                <div class="p-6 rounded-2xl bg-white shadow-lg border border-slate-100">
                    <h3 class="text-xl font-semibold text-slate-900">Layanan Sosial & Kesehatan</h3>
                    <p class="mt-2 text-slate-600">Posyandu, bantuan sosial terintegrasi, serta program kesehatan lansia.</p>
                    <span class="inline-block mt-4 px-3 py-1 bg-blue-50 text-blue-700 text-sm rounded-full">Kesehatan</span>
                </div>
            </div>
        </section>

        <section id="program" class="grid lg:grid-cols-3 gap-8 items-stretch">
            <div class="lg:col-span-2 p-8 rounded-2xl bg-gradient-to-br from-white via-emerald-50 to-emerald-100 border border-emerald-100 shadow-lg">
                ...
                <p class="mt-3 text-slate-700 leading-relaxed">Pengelolaan sampah terpadu, kebun desa organik, dan bank sampah digital yang melibatkan karang taruna serta PKK.</p>
                <ul class="mt-4 space-y-2 text-slate-700">
                    <li class="flex gap-3"><span class="text-emerald-600">•</span> 300 keluarga ikut pemilahan sampah rumah tangga.</li>
                    <li class="flex gap-3"><span class="text-emerald-600">•</span> 12 titik kebun pekarangan kolektif.</li>
                    <li class="flex gap-3"><span class="text-emerald-600">•</span> Insentif poin untuk warga melalui aplikasi desa.</li>
                </ul>
                <div class="mt-6 flex gap-3">
                    <a href="#" class="px-5 py-3 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition">Daftar Relawan</a>
                    <a href="#" class="px-5 py-3 rounded-lg border border-emerald-300 text-emerald-800 font-semibold hover:bg-white transition">Lihat Detail</a>
                </div>
            </div>
        ...
        <section id="kontak" class="grid md:grid-cols-2 gap-10 items-center">
            <div class="space-y-4">
                ...
                <p class="text-slate-700">Hubungi sekretariat {{ $identity->nama_desa ?? 'desa' }} untuk layanan publik, kolaborasi program, atau permintaan data terbuka.</p>
            ...
            <div class="p-8 rounded-2xl bg-white shadow-lg border border-slate-100 space-y-4">
                ...
                <form class="space-y-3">
                    <input type="text" placeholder="Nama" class="w-full border border-slate-200 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    <input type="email" placeholder="Email" class="w-full border border-slate-200 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                    <textarea rows="3" placeholder="Pesan" class="w-full border border-slate-200 rounded-lg px-4 py-3 focus:ring-2 focus:ring-emerald-500 focus:outline-none"></textarea>
                    <button type="button" class="w-full px-4 py-3 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition">Kirim Pesan</button>
                </form>
            </div>
        </section>
    </main>
